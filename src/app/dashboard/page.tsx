...
import {Activity, AlertTriangle, Clock, TrendingUp, Ratio, LogOut, ShieldCheck, ArrowRight, ShieldX, UserCog, CalendarClock, BarChart3, Calculator} from 'lucide-react';
  ...
  const fetchPrice = useCallback(async () => {
    try {
      ...
      setPriceData(prevData => {
        ...
        const change = prevData?.price ? newPrice - prevData.price : 0;
        const changePercent = prevData?.price ? (change / prevData.price) * 100 : 0;
  ...
  const handleGenerateSignal = async () => {
    ...
    if (!priceData) {
      toast({
        variant: 'destructive',
        title: 'Error',
        description: 'Live price data is not available to generate a signal.',
      });
      return;
    }
    ...
    try {
      const signalInput: TradingSignalInput = {
      ...
      if (newSignal && signalsCollection) {
        ...
        toast({
         ...
         throw new Error('Signal generation returned an invalid result.');
      }
    } catch (error: any) {
      console.error(error);
      toast({
        ...
        description: error.message || 'Could not generate a new trading signal. Please try again.',
      });
    } finally {
        ...
        <header className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            ...
            <div className='flex items-center justify-between md:justify-end gap-4 w-full md:w-auto'>
                 <Link 
                    ...
                    className="flex items-center space-x-2 p-2 hover:bg-muted rounded-lg text-sm"
                    >
                    <BarChart3 className="w-5 h-5" />
                    <span className="hidden sm:inline">Signal Backtesting</span>
                </Link>
